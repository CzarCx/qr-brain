...
export default function SewingStatusScannerPage() {
  ...
  const fetchPpcOrders = async () => {
    setLoadingPpc(true);
    try {
      // 1. Fetch from 'personal' table with status PPC
      const { data: personalPpc, error: pError } = await supabase
        .from('personal')
          ...
          <div className="space-y-6">
            ...
            <Card className="shadow-md">
              <CardHeader className="flex flex-row items-center justify-between border-b py-3 px-4">
                ...
                <Button variant="ghost" size="sm" onClick={() => setScanLogs([])} className="text-[10px] h-7 font-bold">LIMPIAR</Button>
              </CardHeader>
